class EmTttPoolsLulucfsController < ApplicationController
  layout 'transition_time'
  before_action :set_page, only: %i[show edit update destroy]
  def index
    @luc_names = EmTttPoolsLulucf.all_luc_names
    @submissions = EmTttPoolsLulucf.all_submissions
    @state_name_ens = EmTttPoolsLulucf.all_state_name_ens

    @q = EmTttPoolsLulucf.ransack(params[:q])
    ...
    respond_to do |format|
      format.html
      format.csv { send_data EmTttPoolsLulucf.export(@results), type: 'text/csv; charset=utf-8; header=present', disposition: "attachment; filename=export.csv" }
    end
  end
end
