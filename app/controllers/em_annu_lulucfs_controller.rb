class EmAnnuLulucfsController < ApplicationController
  layout 'annual_emissions'
  before_action :set_page, only: %i[show edit update destroy]

  def index
    @luc_names = EmAnnuLulucf.all_luc_names
    @pool_name_ens = EmAnnuLulucf.all_pool_name_ens
    @substances = EmAnnuLulucf.all_substances
    @submissions = EmAnnuLulucf.all_submissions
    @state_name_ens = EmAnnuLulucf.all_state_name_ens
    
    @q = EmAnnuLulucf.ransack(params[:q])
    ...
    respond_to do |format|
      format.html
      format.csv { send_data EmAnnuLulucf.export(@results), type: 'text/csv; charset=utf-8; header=present', disposition: "attachment; filename=export.csv" }
    end
  end
end
