class EmAnnuPoolsLulucfsController < ApplicationController
  ...
  def index
    @luc_names = EmAnnuPoolsLulucf.all_luc_names
    @submissions = EmAnnuPoolsLulucf.all_submissions
    @state_name_ens = EmAnnuPoolsLulucf.all_state_name_ens
    ...
    respond_to do |format|
      format.html
      format.csv { send_data EmAnnuPoolsLulucf.export(@results), type: 'text/csv; charset=utf-8; header=present', disposition: "attachment; filename=export.csv" }
    end
  end
end
